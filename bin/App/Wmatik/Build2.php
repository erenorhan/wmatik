...
class Build
{
    ...
    public function index($currentWorkDir = '')
    {
        ...
        if (file_exists($workDir . '/_src/index.twig')) {
            ...
            $_SESSION['headerFiles'] = isset($_SESSION['headerFiles']) ? $_SESSION['headerFiles'] : array();
            $_SESSION['footerFiles'] = isset($_SESSION['footerFiles']) ? $_SESSION['footerFiles'] : array();

            $headerFiles = [];
            $footerFiles = [];

            foreach ($_SESSION['headerFiles'] as $key => $value) {
                foreach ($value as $k => $v) {
                    array_push($headerFiles, $v);
                }
            }
            foreach ($_SESSION['footerFiles'] as $key => $value) {
                foreach ($value as $k => $v) {
                    array_push($footerFiles, $v);
                }
            }

            $headersUnique = array_unique($headerFiles);
            $footersUnique = array_unique($footerFiles);

            $headerStr = '';
            $footerStr = '';

             //header js
             foreach ($headersUnique as $k => $v) {
                $file = explode('.', $v);
                if (end($file) == 'js') {
                    $headerStr .= "    <script src=\"/static/build/" . $v . "\"></script> \n";
                }
            ...
            foreach ($headersUnique as $k => $v) {
                $file = explode('.', $v);
                if (end($file) == 'css') {
                    $headerStr .= "    <link rel=\"stylesheet\" href=\"/static/build/" . $v . "\" /> \n";
                }
            ...
            //footer css
            foreach ($footersUnique as $k => $v) {
                $file = explode('.', $v);
                if (end($file) == 'css') {
                    $footerStr .= "    <link rel=\"stylesheet\" href=\"static/build/" . $v . "\" /> \n";
                }
            ...
            foreach ($footersUnique as $k => $v) {
                $file = explode('.', $v);
                if (end($file) == 'js') {
                    $footerStr .= "    <script src=\"static/build/" . $v . "\"></script> \n";
                }
